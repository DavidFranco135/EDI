import { Document, TimberItem, Bloco, ProductItem, CompensadoItem } from '../types';
import { calcDerived } from './calc';
import { calcDerivedCompensado } from './calcCompensado';
...
interface DocHTMLParams {
  ...
  compensadoItems?: CompensadoItem[];
}

export function buildDocHTML(p: DocHTMLParams): string {
  const { doc, type, totals, commission, total, displayDate, client, settings: s, cheques = [], blocos = [], eco = false, extras = [], extrasTotal = 0, productItems = [], compensadoItems = [] } = p;
  ...
  // ── Compensado table (madeirite plastificado, por chapa) ────────────────
  const compensadoTableHTML: string = doc.docMode === 'compensado' ? (() => {
    const rows = compensadoItems.map((it: CompensadoItem, i: number) => {
      ...
      return (
        ...
        '<td style="' + TD + ';font-weight:bold">' + it.qty + '</td>' +
        '<td style="' + TD + '">' + (d.areaTotal > 0 ? d.areaTotal.toFixed(2) : '—') + '</td>' +
        '<td style="' + TD + ';font-weight:bold;color:#92400e">' + fmt(it.pricePerChapa) + '</td>' +
        '<td style="border:1px solid #e8d090;padding:3px 6px;text-align:right;font-weight:bold;background:' + VAL_BG + '">' + fmt(d.value) + '</td>' +
        '</tr>'
      );
    });
    const areaGrandTotal = compensadoItems.reduce((s: number, it: CompensadoItem) => s + calcDerivedCompensado(it).areaTotal, 0);
    const grandTotal = compensadoItems.reduce((s: number, it: CompensadoItem) => s + calcDerivedCompensado(it).value, 0);
    return (
      '<table style="margin-bottom:4px;font-size:12px">' +
      '<thead><tr style="background:' + TH_BG + ';color:' + TH_TXT + '">' +
      ...
      '<th style="' + TH + ';width:60px">Qtd</th>' +
      '<th style="' + TH + ';width:80px">Área (m²)</th>' +
      '<th style="' + TH + ';width:100px">Preço/Chapa</th>' +
      '<th style="border:1px solid ' + C_GOLD + ';padding:5px 6px;text-align:right;font-weight:bold;font-size:13px;background:' + C_GOLD + ';color:' + C_DARK + ';width:110px">TOTAL</th>' +
      '</tr></thead>' +
      '<tbody>' + rows.join('') + '</tbody>' +
      '<tfoot><tr style="background:' + TOT_BG + ';color:' + TOT_T + ';font-weight:bold;border-top:3px solid ' + C_GOLD + '">' +
  ...
  let html = '<!DOCTYPE html>\n' +
    ...
    // Tables — product, compensado or timber mode
    (doc.docMode === 'produtos' ? productTableHTML
      : doc.docMode === 'compensado' ? compensadoTableHTML
      : tablesSections);
