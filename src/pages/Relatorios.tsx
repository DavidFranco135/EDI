...
import { FileText, Truck, Trash2, ExternalLink, Search, Calendar, CheckCircle2, Clock, DollarSign, AlertCircle, Eye } from 'lucide-react';
  ...
  return (
    <div className="space-y-5 pb-24 md:pb-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        ...
        {(typeFilter === 'pedido' || typeFilter === 'todos') && (
          <div className="space-y-0.5">
            <label className="text-[9px] font-bold text-gray-400 uppercase tracking-wider px-0.5">Status Pedido</label>
        ...
        {filtered.map(doc => (
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 flex-shrink-0">
              ...
              <button
                onClick={() => openDocumentPreview(doc, state.settings, state.clients)}
                title="Visualizar exatamente como sai no PDF"
                className="flex items-center justify-center gap-1.5 px-3 py-2 sm:py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-xs font-bold transition-all"
              >
                <Eye className="w-3.5 h-3.5" /> <span className="sm:hidden">Ver</span>
              </button>
              <Link
