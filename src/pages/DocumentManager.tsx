...
import { CompensadoCalculator } from '../components/CompensadoCalculator';
import { totalAreaCompensado } from '../lib/calcCompensado';
...
import { ArrowLeft, Save, Printer, Plus, Trash2, ChevronDown, ChevronUp, Building2, Upload, CheckCircle2, AlertCircle, Users, FileCheck, Copy, Eye } from 'lucide-react';
...
export const DocumentManager: React.FC<{ type: 'pedido' | 'romaneio' }> = ({ type }) => {
  ...
  const totals = useMemo(() => {
    ...
    if (doc.docMode === 'compensado') {
      const subtotal = (doc.compensadoItems || []).reduce((s, it) => s + it.qty * it.pricePerChapa, 0);
      return { m3: 0, subtotal };
    }
  ...
  }, [doc.blocos, doc.productItems, doc.compensadoItems, doc.docMode]);
  ...
  const confirmDuplicate = async (markAsReference: boolean) => {
    ...
    try {
      ...
      const rawDuplicated: any = {
        ...
        compensadoItems: (doc.compensadoItems || []).map(it => ({ ...it, id: Math.random().toString(36).slice(2, 9) })),
      ...
      navigate(`/${type === 'pedido' ? 'pedidos' : 'romaneios'}/${duplicated.id}`);
    } catch (err: any) {
    ...
    <div className="space-y-5 pb-32 max-w-full overflow-x-hidden">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handlePrint}
            title="Visualizar exatamente como sai no PDF"
            className="flex-shrink-0 flex items-center justify-center w-9 h-9 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 active:scale-95 transition-all">
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            {!showDocFields && (
              <span className="text-[10px] text-gray-400 font-bold">
                Nº {doc.number} · {doc.woodType || 'pinus'} · {doc.docMode === 'produtos' ? 'Produtos' : doc.docMode === 'compensado' ? 'Compensado' : 'Madeira'}
              </span>
            )}
          </div>
        ...
        <div className="space-y-1 md:col-span-2 lg:col-span-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <button type="button"
              onClick={() => setDoc(p => ({ ...p, docMode: 'compensado' }))}
              className={[
                'py-2.5 rounded-lg text-sm font-bold border-2 transition-all flex items-center justify-center gap-2',
                doc.docMode === 'compensado'
      ...
      {doc.docMode === 'produtos' ? (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm overflow-x-auto">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-sm font-black text-orange-700">🪚 Compensado / Madeirite Plastificado</span>
            </div>
            <CompensadoCalculator
              items={doc.compensadoItems || []}
              onChange={compensadoItems => setDoc(p => ({ ...p, compensadoItems }))}
            />
          </div>
        </div>
      ) : (
        ...
        <div className="grid grid-cols-2 gap-3 text-sm mb-3">
          <div>
            <p className="text-green-300 text-xs font-bold uppercase tracking-wider">
              {doc.docMode === 'compensado' ? 'Total m²' : 'Total M³'}
            </p>
            <p className="text-xl font-black">
              {doc.docMode === 'compensado'
                ? totalAreaCompensado(doc.compensadoItems || []).toFixed(2)
                : totals.m3.toFixed(3)}
            </p>
          </div>
          <div>
